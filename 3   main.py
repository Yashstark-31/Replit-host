...
ADMIN_ID = 6716979039 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = '@Dhruv0757' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 'https://t.me/update69xd' # Replace with your update channel link
...
FORCE_JOIN_CHANNEL = "CloudXSupport_channel"   # Without @
FORCE_JOIN_GROUP   = "CloudXSupport_Group"      # Without @

def is_user_joined(user_id):
    """Returns True if user has joined BOTH channel and group, else False."""
    joined_statuses = {'member', 'administrator', 'creator'}
    try:
        ch = bot.get_chat_member(f"@{FORCE_JOIN_CHANNEL}", user_id)
        if ch.status not in joined_statuses:
            return False
    except Exception:
        return False
    try:
        gr = bot.get_chat_member(f"@{FORCE_JOIN_GROUP}", user_id)
        if gr.status not in joined_statuses:
            return False
    except Exception:
        return False
    return True
...
def send_force_join_message(chat_id):
    """Send the join-required message with inline buttons."""
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("✅ I've Joined – Verify", callback_data="check_join"),
    )
    bot.send_message(
        chat_id,
        "⚠️ *Access Restricted!*\n\n"
        ...
        "2️⃣ Join the Group\n"
        "3️⃣ Click *✅ I've Joined – Verify*",
        reply_markup=markup,
        parse_mode='Markdown'
    )

# --- Pending Approval Data (in-memory) ---
# {pending_id: {'user_id':..,'file_name':..,'file_type':..,'file_data':bytes,'chat_id':..,'timestamp':..}}
pending_approvals = {}
_pending_counter  = 0
...
def _logic_send_welcome(message):
    ...
    # ── FORCE JOIN CHECK (admins/owner bypass) ──────────────────
    if user_id not in admin_ids and not is_user_joined(user_id):
...
def _logic_pending_files(message):
    """Admin: show all pending approval requests."""
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "⚠️ Admin permissions required.")
        return
    if not pending_approvals:
        bot.reply_to(message, "⏳ No pending file approvals right now.")
        return
    for pid, info in list(pending_approvals.items()):
        approve_markup = types.InlineKeyboardMarkup(row_width=2)
        approve_markup.add(
        ...
        bot.send_message(
            message.chat.id,
            f"📄 *Pending File*\n👤 User: `{info['user_id']}`\n📄 File: `{info['file_name']}`\n"
            f"🗂️ Type: `{info['file_type'].upper()}`\n🕐 Time: {info['timestamp']}",
            reply_markup=approve_markup, parse_mode='Markdown'
        )
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "⏳ Pending Files": _logic_pending_files,        # Admin: approve/reject uploads
}
...
def handle_file_upload_doc(message): # Renamed
    ...
    try:
        ...
        bot.edit_message_text(f"✅ Downloaded `{file_name}`.", chat_id, download_wait_msg.message_id, parse_mode='Markdown')
        logger.info(f"Downloaded {file_name} for user {user_id}")

        # ── ADMIN UPLOADS: host directly (no approval needed) ────
        if user_id in admin_ids:
            user_folder = get_user_folder(user_id)
            if file_ext == '.zip':
                ...
                logger.info(f"Admin: saved file to {file_path}")
        ...
        # ── REGULAR USER: send to ALL admins for approval ────────
        pending_id = _new_pending_id()
        file_type  = file_ext.lstrip('.')   # 'py', 'js', or 'zip'
        pending_approvals[pending_id] = {
            'user_id'  : user_id,
            'file_name': file_name,
            'file_type': file_type,
            'file_data': downloaded_file_content,
            'chat_id'  : chat_id,
        ...
        bot.send_message(
            chat_id,
            f"📨 Your file `{file_name}` has been sent to admin for *approval*.\n"
        ...
        for aid in list(admin_ids):
            try:
                # Forward the actual file so admin can inspect it
                bot.forward_message(aid, chat_id, message.message_id)
                bot.send_message(aid, admin_caption, reply_markup=approve_markup, parse_mode='Markdown')
            except Exception as e:
                logger.error(f"Failed to notify admin {aid} for pending file: {e}")
        # ────────────────────────────────────────────────────────
...
def handle_callbacks(call):
    ...
    if bot_locked and user_id not in admin_ids and data not in ['back_to_main', 'speed', 'stats']: # Allow stats
        ...
        return
    try:
        # ── FORCE JOIN VERIFY button ─────────────────────────────
        if data == 'check_join':
            if is_user_joined(user_id):
                try: bot.delete_message(call.message.chat.id, call.message.message_id)
                except Exception: pass
                bot.answer_callback_query(call.id, "✅ Verified! Welcome!", show_alert=False)
                bot.send_message(call.message.chat.id, "✅ *Verification successful!* You can now use the bot.", parse_mode='Markdown')
                _logic_send_welcome(call.message)
            else:
                bot.answer_callback_query(call.id, "❌ You haven't joined yet! Please join both and try again.", show_alert=True)
            return
        # ── ADMIN APPROVE file ───────────────────────────────────
            ...
            if user_id not in admin_ids:
                ...
                return
            pending_id = data[len('approve_'):]
            info = pending_approvals.get(pending_id)
            if not info:
                bot.answer_callback_query(call.id, "⚠️ Request not found or already handled.", show_alert=True)
                return
            bot.answer_callback_query(call.id, "✅ Approved! Hosting now...")
            # Remove buttons from admin message
            try: bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
            except Exception: pass
            bot.send_message(call.message.chat.id, f"✅ Approved `{info['file_name']}` for user `{info['user_id']}`.", parse_mode='Markdown')
            ...
            # Build a minimal message-like object to reuse existing handlers
            class _FakeMsg:
                class _Chat: pass
                class _User: pass
                def __init__(self, cid, uid, fname):
                    self.chat = self._Chat(); self.chat.id = cid
                    self.from_user = self._User(); self.from_user.id = uid
                    self.from_user.first_name = str(uid)
                    self.message_id = 0; self.text = fname

            fake_msg = _FakeMsg(target_chat, target_uid, file_name)

            try:
                if file_type == 'zip':
                    handle_zip_file(file_data, file_name, fake_msg)
                else:
                    file_path = os.path.join(user_folder, file_name)
                    with open(file_path, 'wb') as fh: fh.write(file_data)
                    if file_type == 'js': handle_js_file(file_path, target_uid, user_folder, file_name, fake_msg)
                    elif file_type == 'py': handle_py_file(file_path, target_uid, user_folder, file_name, fake_msg)
                # Notify the user
                try:
                    bot.send_message(target_chat, f"✅ Your file `{file_name}` was *approved* by admin and is now running!", parse_mode='Markdown')
                except Exception: pass
            except Exception as ex:
                logger.error(f"Error hosting approved file {file_name}: {ex}", exc_info=True)
                bot.send_message(call.message.chat.id, f"❌ Error while hosting: {ex}")
        ...
        # ── ADMIN REJECT file ────────────────────────────────────
            ...
            bot.send_message(call.message.chat.id, f"❌ Rejected `{info['file_name']}` from user `{info['user_id']}`.", parse_mode='Markdown')
            try:
                bot.send_message(info['chat_id'], f"❌ Your file `{info['file_name']}` was *rejected* by admin and will not be hosted.", parse_mode='Markdown')
            except Exception: pass
            del pending_approvals[pending_id]
            return
        # ── ADMIN: list pending files ────────────────────────────
            ...
            if not pending_approvals:
                bot.send_message(call.message.chat.id, "⏳ No pending file approvals right now.")
                return
            for pid, info in list(pending_approvals.items()):
                approve_markup = types.InlineKeyboardMarkup(row_width=2)
                approve_markup.add(
